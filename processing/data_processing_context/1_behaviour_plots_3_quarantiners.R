behaviourPlot7SocialDistancing <- function(plot_specific_f_name) {
  ...
  print(paste("Social distancing average, ", plot_base_name ,": ", mean(df_population_status$social_distancing)))
  df_population_status <- gather(df_population_status, `Population Status`, measurement, people_alive, social_distancing)
  
  p <- ggplot(df_population_status, aes(x = tick, y = measurement, col=`Population Status`)) + geom_line()
  p <- p + scale_colour_manual(
    labels=c('people_alive'='people_alive',
             'social_distancing' ='social_distancing'),
    values=c('#2269ee', '#00A300'),
    breaks=c('people_alive','social_distancing'))
