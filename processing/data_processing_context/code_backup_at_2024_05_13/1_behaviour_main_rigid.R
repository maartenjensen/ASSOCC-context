...
rigid_norm = unique(df_final$ce_should_rigidly_follow_quarantine)[1]   # Check the depth values and make dependent on the dataframe's depth levels
for (rigid_norm in unique(df_final$ce_should_rigidly_follow_quarantine))
{
  if (rigid_norm)
  { title_name = "rigid" }
  else
  { title_name = "not_rigid" }

  depth_value = 3 # Since its about checking the rigid norms
  subset_df <- df_final_filtered[df_final_filtered$ce_should_rigidly_follow_quarantine == rigid_norm, ]
  ...
  df_deliberation_type <- select(subset_df, tick, ce_should_rigidly_follow_quarantine, people_alive, `Minimal context`, `Most salient need`, `Compare need levels`, `Normative deliberation`, `Conformity deliberation`, `Full need`)
  ...
  df_population_status <- select(subset_df, tick, ce_should_rigidly_follow_quarantine, uninfected, infected, believe_infected, dead_people, healthy) #, immune, believe_immune, 
  ...
  df_needs <- select(subset_df, tick, ce_should_rigidly_follow_quarantine, autonomy, belonging, compliance, conformity, financial_stability,
  ...
  df_location_types <- select(subset_df, tick, ce_should_rigidly_follow_quarantine, at_essential_shops, at_homes, at_non_essential_shops,
