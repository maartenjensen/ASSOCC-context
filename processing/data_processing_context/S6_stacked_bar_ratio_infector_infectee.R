...
plotS6StackedBarRatioInfectorInfectee <- function(df_scenario6, output_dir, one_plot) {
  
  name = "s6_stacked_bar_ratio_infector_infectee"
  ...
  df_list_ratio_infections = split(df_ratio_infections, f = df_ratio_infections$ratio_of_app_users)
  ...
  for (i in 1:length(df_list_wide_ratio_infections)) {
    ...
    t_w_r_i <- t_w_r_i %>% 
      select(ratio_of_app_users:raw_summed_ratio) %>% 
  ...
  for (i in 1:length(df_list_wide_ratio_infections_real)) {
    ...
    #df_plot$color <- df_plot %>% select(infector)
    #names(df_plot[, 8]) = "color"
    #df_plot$color <- recode_factor(df_plot$color, 'Young' = '#cedecb',  'Student' = '#de9236',
    #                                              'Worker' = '#16727e', 'Retired' = '#391c1c')
    
    p_title = paste("Ratio infectors per infectee - app usage ratio: ", mean(df_plot$ratio_of_app_users), sep="")
    dmfPdfOpen(output_dir, paste("s6_ratio_infector_infectee_app_", mean(df_plot$ratio_of_app_users), sep=""))
...
plot_ggplot_bar <- function(data_to_plot, p_title = "None") {
  ...
  data_to_plot %>% ggplot(aes(x=infectee, y=real_ratio, fill=infector)) +
    ...
    geom_text(aes(y=1-label_ypos, label=round(real_ratio, digits=3), vjust="middle"), 
              color="white", size=5) +
    #scale_fill_brewer(palette=palette(), name="Infector age") +
    labs(title=p_title,
         caption="Agent-based Social Simulation of Corona Crisis (ASSOCC)",
         x="Infectee age group", y="Ratio infectors") +
    gl_plot_theme +
    scale_fill_manual(values=c('#cedecb','#de9236', '#16727e','#391c1c'))
}