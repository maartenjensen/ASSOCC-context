...
plotS6ContactsAtGatheringPoint <- function(df_scenario6, output_dir, one_plot) {
  ...
  name = "s6_contacts_at_location_type"
...
plot_ggplot <- function(data_to_plot, app_use) {
  ...
  data_to_plot %>%
    ...
    labs(title=paste("Number of contacts per location type - app usage ratio:", app_use),
...
plot_ggplot_smooth <- function(data_to_plot, app_use) {
  ...
  data_to_plot %>%
    ...
    xlab("Days") +
    ylab("Number of contacts per day") +
    labs(title=paste("Number of contacts per location type (Smoothed) - app usage ratio:", app_use),
         caption="Agent-based Social Simulation of Corona Crisis (ASSOCC)") +
    gl_plot_guides + gl_plot_theme
}
...
plot_ggplot_bar <- function(data_to_plot) {
  ...
  data_to_plot %>%
    ggplot(aes(x=Location_type, y=measurement, fill=as.factor(ratio_of_app_users)), fill=NA) +
    ...
    gl_plot_guides + gl_plot_theme_side +
