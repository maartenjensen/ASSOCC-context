...
plotS6StackedBarRatioContactsPerContacted <- function(df_scenario6, output_dir, one_plot) {
  
  name = "s6_stacked_bar_ratio_contacts_contacted"
  ...
  df_list_ratio_contacts = split(df_ratio_contacts, f = df_ratio_contacts$ratio_of_app_users)
  ...
  for (i in 1:length(df_list_wide_ratio_contacts_real)) {
    ...
    p_title = paste("Ratio contactors per contacted - app usage ratio: ", mean(df_plot$ratio_of_app_users), sep="")
    dmfPdfOpen(output_dir, paste("s6_ratio_contactor_contacted_app_", mean(df_plot$ratio_of_app_users), sep=""))
