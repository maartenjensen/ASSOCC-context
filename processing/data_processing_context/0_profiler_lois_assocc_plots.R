...
plot_time_comparison_original_assocc <- function(df_time_original_assocc, directory_files, n_experiments_active, plot_type) {
  p <- ggplot(df_time_original_assocc, aes(x = Agents, y = incl_t_ms_mean, 
    ...
    geom_line(linewidth = 1.2) +
    geom_point(size = 2.5, show.legend = FALSE) +
    labs(title = paste("Unoptimised ASSOCC - Time Comparison (r = 2)", sep = ""),
         x = "Agents at start",
         y = "Incl execution time (ms)",
         color = "Time Type",
         fill = "Time Type",
  ...
  if (plot_type == "one") { safe_to_pdf(paste("plot_", directory_files, "_comparison_unoptimised_assocc.pdf", sep=""), p) }
  else { show(p) }
}
